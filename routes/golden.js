...
router.post('/', function(req, res) {
  ...
  pool.query('INSERT INTO golden(filename, url, description, criteria_array, creation_date, passfail, explanation) VALUES($1, $2, $3, $4, $5, $6, $7)', [data.filename, data.url, data.description, data.criteria_array, data.creation_date, data.passfail, data.explanation], function(err, result) {
    // handle an error from the query
    if (err) {return res.json(err);}
    // console.log(result.rows);
    res.json(result);
